...
define([
  ...
  function(declare, lang, query, html, array, fx, all, Deferred, _WidgetsInTemplateMixin,
    ...
    GraphicsLayer, FeatureLayer, SimpleRenderer, InfoTemplate, symbolJsonUtils, SimpleFillSymbol, SimpleLineSymbol, esriLang,
    esriRequest, scaleUtils, geometryEngine, Point, Extent, webMercatorUtils, SimpleMarkerSymbol, Graphic, Colour, SingleTask, domAttr, sniff) {
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
	  ...
	  //M.Norbury April 2016
	  
	  pointLayer: null,
	  ptLatLong: null,
	  uploadedLayer: null,
	  bufferLayer: null,
	  //cbxBuffer.checked:false,
      ...
      */
      ...
      _combineRadioCheckBoxWithLabel: function(){
		...
		jimuUtils.combineRadioCheckBoxWithLabel(this.cbxInputLatLong, this.inputLatLongLabel);
		jimuUtils.combineRadioCheckBoxWithLabel(this.cbxUploadGraphic, this.uploadGraphicLabel);
		jimuUtils.combineRadioCheckBoxWithLabel(this.cbxBuffer, this.bufferLabel);
	  ...
	  _initUploadBox: function(){
		//This function is called when the query tool is loaded.
	  },
      ...
      _initSelf:function(){
		...
		this.cbxInputLatLong.name = cbxName;
		this.cbxUploadGraphic.name = cbxName;
      ...
      _onCbxUseSpatialClicked: function(){
        ...
        } else if (this.cbxDrawGraphic.checked) {
          this._onCbxDrawGraphicClicked();
		} else if (this.cbxInputLatLong.checked) {
          this._onCbxInputLatLongClicked();
        } else {
          this._onCbxUploadGraphicClicked();
        }

        this._resetDrawBox();
		this._resetUploadBox();
      },
      ...
      _onCbxUseMapExtentClicked: function(){
        if(this.cbxUseMapExtent.checked){
		  ...
		  html.setStyle(this.inputLatLongDiv, 'display', 'none');
		  html.setStyle(this.uploadBoxDiv, 'display', 'none');
		  html.setStyle(this.bufferDiv, 'display', 'none');
      ...
      _onCbxDrawGraphicClicked: function(){
        if(this.cbxDrawGraphic.checked){
		  ...
		  html.setStyle(this.inputLatLongDiv, 'display', 'none');
		  html.setStyle(this.uploadBoxDiv, 'display', 'none');
		  html.setStyle(this.bufferDiv, 'display', 'block');
        }
      },
	  
	  _onCbxInputLatLongClicked: function(){
        if(this.cbxInputLatLong.checked){
		  this._resetDrawBox();
		  this._resetUploadBox();
          html.setStyle(this.drawBoxDiv, 'display', 'none');
		  html.setStyle(this.inputLatLongDiv, 'display', 'block');
		  html.setStyle(this.uploadBoxDiv, 'display', 'none');
		  html.setStyle(this.bufferDiv, 'display', 'block');
        }
      },
	  
	  _onCbxUploadGraphicClicked: function(){
        if(this.cbxUploadGraphic.checked){
		  this._resetDrawBox();
          html.setStyle(this.drawBoxDiv, 'display', 'none');
		  html.setStyle(this.inputLatLongDiv, 'display', 'none');
		  html.setStyle(this.uploadBoxDiv, 'display', 'block');
		  html.setStyle(this.bufferDiv, 'display', 'block');
        }
      },
	  ...
	  _resetUploadBox: function(){
        //Placeholder in case this function is needed.
      },
      ...
      _onBtnApplyClicked:function(){
        ...
        if(this.cbxUseSpatial.checked){
          if(this.cbxUseMapExtent.checked){
			...
			if (this.pointLayer){
				geometry = this.ptLatLong;							
			}
			else {
				new Message({
					message:this.nls.noLatLongPlaced				
				});
				return;
			}
		}		
		else {
			//M.Norbury, April 2016, use uploaded shapefile
			fileName=domAttr.get(this.fileInput, 'value');
			if(!fileName){
				new Message({
				...
				//Unite all the polygons.
				console.log("Uniting the polygons");
				this._slide(this.queryParams, 0, -100);
				array.forEach(this.uploadedLayer.graphics, function (graphic) {
					if (!geometry) {
						//Set the geometry variable to be the first graphic found.
						geometry = graphic.geometry;
					}
					else {
						//Add each graphic in turn to the geometry variable.
						geometry = geometryEngine.union([geometry, graphic.geometry]);
					}				
				});
				if(geometry){
					console.log("Using uploaded layer.");  
				}				  
			}
		  ...
		  if(!this.cbxUseMapExtent.checked){
			  if(this.cbxBuffer.checked){
				if (this.txtBufferDistance.value<=0){
						new Message({message: this.nls.invalidBuffer});
						return;
				}
				else {										
					var bufferDistance = this.txtBufferDistance.value;		
					if(this.cbxInputLatLong.checked){
						console.log("Buffering the point " + this.ptLatLong + " at distance " + bufferDistance + " metres.");
						var bufferedGeometry = geometryEngine.buffer(this.ptLatLong,bufferDistance,'meters',true);
					}
					else {
						console.log("Buffering geometry " + geometry + " at distance " + bufferDistance + " metres.");
						var bufferedGeometry = geometryEngine.geodesicBuffer(geometry,bufferDistance,'meters',true);
					}					
					console.log("Buffered the graphics at " + bufferDistance + " metres, giving " + bufferedGeometry + " geometry.");
					
					var buffSymbol = new SimpleFillSymbol(
						SimpleFillSymbol.STYLE_SOLID,
						new SimpleLineSymbol(
						  SimpleLineSymbol.STYLE_SOLID,
						  new Colour([0,0,255,.65]), 2
						),
						new Colour([0,0,255,.35])
					  );
					if (this.bufferLayer){
						//Remove the old buffer layer
						this.map.graphics.remove(this.bufferLayer);
					}
					this.bufferLayer = new Graphic(bufferedGeometry,buffSymbol);
					this.map.graphics.add(this.bufferLayer);
	  ...
	  _onBuffer: function(){
		  if(this.cbxBuffer.checked){
			  html.setStyle(this.bufferDistance, 'display', 'block');
		  }
		  else {
			  html.setStyle(this.bufferDistance, 'display', 'none');
		  }		  
	  },
	  
	  _onPlacePoint: function(){
		  ...
		  //Centre the map on this point.
		  this.map.centerAt(this.ptLatLong);
	  },
	  ...
	  _onUpload: function(){
		...
		if(!domAttr.get(this.fileInput, 'value')){
			...
			//new Message({message: this.nls.pleaseWait});
			...
			if (fileName.indexOf(".zip") == -1) {
				...
				new Message({message: this.nls.notAZipFile});			  
				return;
			}
			...
			//Generalize features for display.  Here we generalize at 1:40,000 which is approx 10 metres.
			...
			var portalUrl = "https://www.arcgis.com/sharing/rest/content/features/generate";
			esriRequest({
			  url: portalUrl,
			  content: myContent,
			...
			lang.hitch(this, function (data) {
				...
				layer = data.featureCollection.layers[0];
				
				//If a shapefile has previously been uploaded then remove it.
				if (this.uploadedLayer){
					this.map.removeLayer(this.uploadedLayer);
				}
				//Store the layer in the global variable: uploadedLayer
				this.uploadedLayer = new FeatureLayer(layer);
				//Give the layer a name for the layer list.
				this.uploadedLayer.name = "Query filter";	
				//Add the shapefile to the map
				this.map.addLayer(this.uploadedLayer);	
				//Zoom to the feature collection extent
				this.map.setExtent(this.uploadedLayer.fullExtent.expand(1.25), true);	
				console.log(this.nls.uploadSuccess);
			}),
			lang.hitch(this, function (error) {
				console.log("Layer upload has failed.  Error: ", error.message);
				new Message({message:this.nls.uploadFail});				
			  })
			);
